...
- **문자열 + 숫자를 그냥 더하면 `TypeError`**가 나. 자바는 `"결과: " + 1`처럼 자동으로 문자열 변환을 해주지만, 파이썬은 `str`과 `int`를 명시적으로 맞춰줘야 해 (`"결과: " + str(1)`). 이럴 땐 `f"결과: {1}"`처럼 **f-string**을 쓰면 타입 캐스팅 신경 안 써도 돼서 훨씬 편해.
